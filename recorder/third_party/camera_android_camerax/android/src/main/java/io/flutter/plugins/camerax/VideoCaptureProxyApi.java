...
class VideoCaptureProxyApi extends PigeonApiVideoCapture {
  ...
  public VideoCapture<?> withOutput(
    ...
    // AKCES BOOTH FORK: wlacz video stabilization (EIS podczas recording).
    // CameraX ignoruje jesli urzadzenie nie wspiera (wg docs).
    try {
      builder.setVideoStabilizationEnabled(true);
    } catch (Throwable t) {
      android.util.Log.w("AkcesBoothFork",
          "setVideoStabilizationEnabled fail: " + t.getMessage());
    }
